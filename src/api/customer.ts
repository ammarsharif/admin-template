...
// types
import { CustomerList, CustomerListMinimal, CustomerProps } from 'types/customer';
...
export async function insertCustomer(newCustomer: CustomerListMinimal) {
...
export async function updateCustomer(customerId: number, updatedCustomer: CustomerListMinimal) {
  // to update local state based on key
  mutate(
    endpoints.key + endpoints.list,
    (currentCustomer: any) => {
...
export function handlerCustomerDialog(modal: boolean) {
  // to update local state based on key
  console.log('handle customer modal');
  console.log(modal);
